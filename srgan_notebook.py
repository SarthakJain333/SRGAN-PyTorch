...
with ZipFile('/content/drive/MyDrive/srgan_10k.zip', 'r') as zObject:
    zObject.extractall()
...
n_epochs = 5
# name of the dataset
dataset_path = '/content/srgan_10k'
# size of the batches
batch_size = 16
# adam: learning rate
lr = 0.00008
...
for epoch in range(n_epochs):
    ...
    for batch_idx, imgs in enumerate(tqdm_bar):
        ...
        if random.uniform(0,1)<0.1:
            ...
            gen_hr = make_grid(gen_hr*255.0, nrow=1, normalize=True)
    ...
    if np.argmin(test_gen_losses) == len(test_gen_losses)-1:
        torch.save(generator.state_dict(), "saved_models/generatorfinal.pth")
        torch.save(discriminator.state_dict(), "saved_models/discriminatorfinal.pth")
